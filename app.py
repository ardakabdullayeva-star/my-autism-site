...
# Список ролей на русском для логики сопоставления в базе
REQUIRED_ROLES_RU = ["Учитель", "Психолог", "Логопед", "Дефектолог"]

# --- ВЫБОР ЯЗЫКА В БОКОВОЙ ПАНЕЛИ ---
with st.sidebar:
    st.markdown("### 🌐 Тіл / Язык")
    language = st.radio("Выберите язык интерфейса:", ["Русский", "Қазақша"], label_visibility="collapsed")
    T = lang_dict[language]

# Маппинг выбранной роли на русский язык для сохранения в БД
role_mapping = {
    T["roles"][0]: "Учитель",
    T["roles"][1]: "Психолог",
    T["roles"][2]: "Логопед",
...
if not st.session_state.logged_in:
    # Центрирование окна входа на экране
    col1, col2, col3 = st.columns([1, 1.3, 1])
    with col2:
        st.markdown("<div class='login-box'>", unsafe_with_html=True)
        st.h3 = st.markdown(f"<h3 style='text-align: center; margin-top:0;'>{T['login_header']}</h3>", unsafe_with_html=True)
        ...
        if st.button(T['login_btn'], use_container_width=True, type="primary"):
            if pwd_input == "12345":
                ...
                st.session_state.role = role_mapping[role_input]  # Сохраняем всегда на RU для логики
                st.session_state.role_display = role_input        # Для отображения
    ...
    # Если вход выполнен成功 — отображаем систему
    with st.sidebar:
        st.write("---")
    ...
    with tabs[0]:
        child_id = st.text_input(T['child_id'], "Иванов Иван / Иванов Иван")
        st.write("---")
        
        score = 0
        details = ""

        # --- ТЕСТЫ НА ДВУХ ЯЗЫКАХ ---
        if language == "Русский":
    ...
    with tabs[1]:
        ...
        if not all_data.empty:
            target_child = st.selectbox(T['child_id'], all_data['child_id'].unique(), key="report_child_select")
            ...
            if not missing:
                ...
                if st.button(T['gen_report'], use_container_width=True, type="primary"):
                    ...
                    # Логика рекомендаций ИИ в зависимости от выбранного языка
                    if language == "Русский":
                        if total_score >= 20:
                            risk_level = "ВЫСОКИЙ"
                            ...
                            risk_level = "НИЗКИЙ"
                            advice = "Выраженных признаков не обнаружено. Рекомендовано плановое педагогическое наблюдение."
                    else:  # ТЕКСТ НА КАЗАХСКОМ
                        if total_score >= 20:
                            risk_level = "ЖОҒАРЫ"
                    ...
                    if language == "Русский":
                        ...
                        for _, row in child_results.iterrows():
                            # Переводим роль на казахский для отчета
                    ...
                    st.download_button(
                        label=T['download_button' if 'download_button' in T else 'download_btn'],
                ...
                # Переводим недостающие роли на язык интерфейса
